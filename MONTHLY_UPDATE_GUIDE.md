...
**Note:** Hotels where the API cannot match the name will have `null` for enriched fields. Typically ~95%+ of hotels are matched automatically via multi-strategy matching and name-based API fallback.
...
The primary extraction bookmarklet resolves ~95%+ of hotel IDs via multi-strategy matching and name-based API fallback. A small percentage may still fail due to name mismatches between the lounge page and the API. Common mismatch patterns include:
