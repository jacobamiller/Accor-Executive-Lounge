...
const EXECUTIVE_LOUNGE_HOTEL_IDS = new Set([
  ...
  // ===== PLACEHOLDER: Additional global hotel IDs =====
  // After running the "Extract All Lounge Hotels" bookmarklet,
  // paste all resolved hotel IDs here organized by region/country.
  // The bookmarklet output includes hotel_id as the 5th element
  // of each hotel array. Extract non-null IDs and add them below.
  //
  // To generate the full list from extraction data:
  //   1. Run the "Extract All Lounge Hotels" bookmarklet
  //   2. Save the JSON output
  //   3. Use this command to extract IDs:
...
const EXECUTIVE_LOUNGE_HOTEL_IDS = new Set([
  ...
  //
  // Expected: ~256 IDs (83% of 309 hotels matched via API)
  // Remaining ~53 hotels need manual lookup on all.accor.com
]);
...
// Watch for SPA route changes — activate on ALL Accor booking pages (global)
(function watchRouteChanges() {
  let lastUrl = location.href;
  setInterval(() => {
    if (location.href === lastUrl) return;
    lastUrl = location.href;
    // Match any Accor booking page (expanded from Vietnam-only to global)
    const isTarget = /all\.accor\.com\/booking/i.test(location.href);
    stopObserver();
    if (isTarget) init();
  }, 1000);
