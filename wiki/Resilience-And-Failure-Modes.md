...
| Everything broken on already-open tabs after extension reload | Content scripts orphaned ("Extension context invalidated") | Close tab and reopen; `safeSendMessage` warns once, then `shutdownExtension()` stops the route-watch interval + mutation observer so the dead script goes quiet (no repeated warnings) |
